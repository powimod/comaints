...
class AccountRoutes {
    ...
    initialize(expressApp) {
        ...
        expressApp.get('/api/v1/account/profile', requireUserAuthMiddleware, async (request, _) => {
        ...
        expressApp.post('/api/v1/account/change-password', requireUserAuthMiddleware, async (request, _) => {
            const view = request.view;
            try {
                const userId = request.userId;
                assert(userId !== null);
        ...
        expressApp.post('/api/v1/account/change-email', requireUserAuthMiddleware, async (request, _) => {
            const view = request.view;
            try {
                const userId = request.userId;
                assert(userId !== null);
